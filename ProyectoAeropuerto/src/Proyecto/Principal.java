...
public static Vuelo[] buscarVuelosOrigenDestino(String origen,String destino, Aeropuerto aeropuertos[]) {
	...
	for(int i=0;i<aeropuertos.length;i++) {//recorre los aeropuertos
		for(int j=0;j<aeropuertos[i].getNroCompañia();j++) {//recorre las compañias
			for(int k=0;k<aeropuertos[i].getCompañia(j).getNumVuelo();k++) {//recorre los vuelos
	...
	for(int i=0;i<aeropuertos.length;i++) {
		for (int j = 0; j <aeropuertos[i].getNroCompañia(); j++) {
